...
export default (str, path) => str.split(';\n')
  ...
  .map(item => item
    ...
    return p.concat({node: `uuu-${path}.${c[0].join('.')}`, value: c[1].toString()})
  }, []).forEach(item => {
...
//      .split each item that has a '-'' into another sub-array
...
// .forEach over the array to run the function that we need
