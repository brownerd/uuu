...
import promiseToWriteFontSize from './promise-to-write-font-size';

const UI_PATH = `${atom.packages.getLoadedPackage('uuu-ui').path}/styles/uuu-font-sync.less`;

let formatFontSize = () => `@font-size: ${atom.config.get('editor.fontSize')}px;`;
let formatLineHeight = () => `@component-line-height: ${atom.config.get('editor.lineHeight')};`;
...
export default function () {
  promiseToWriteFontSize(UI_PATH, data, 'utf8').then( () => {
