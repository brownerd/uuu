'use babel';

export default {
  theme: {
    ui: `@text-color: rgba(18, 18, 18, 1);
@text-value: 0;
@heading-color: rgba(237, 237, 245, 1);
@heading-value: 0;
@background-color: rgba(156, 157, 162, 1);
@background-value: 0;
@background-navvalue: -30;
@button-color: rgba(250, 62, 45, 1);
@button-value: 0;
@button-textcolor: text;
@border-value: -25;
@border-radius: 2;
@git-infoColor: rgba(0, 170, 255, 1);
@git-successColor: rgba(0, 255, 102, 1);
@git-warningColor: rgba(255, 213, 0, 1);
@git-errorColor: rgba(255, 0, 0, 1);
@settingsPage-background: solid;
@settingsPage-nav: horizontal;
@settingsPage-border: false;
@terminal-backgroundColor: rgba(117, 118, 124, 1);
@terminal-backgroundValue: -50;
@terminal-textColor: rgba(255, 255, 255, 1);
@terminal-cursorColor: rgba(232, 79, 60, 1);`,
    syntax: `@editor-backgroundcolor: rgba(255, 255, 255, 1);
@editor-color: rgba(18, 18, 18, 1);
@gutter-color: rgba(156, 157, 162, 1);
@gutter-value: 30;
@gutter-punk: hellyeah;
@selectedline-backgroundcolor: rgba(255, 126, 121, 1);
@selectedline-backgroundguttercolor: rgba(255, 57, 8, 1);
@cursor-style: pacman;
@cursor-color: rgba(255, 213, 0, 1);
@cursor-shadow: rgba(51, 51, 51, 1);
@indentguide-linecolor: rgba(156, 157, 162, 1);
@indentguide-value: 30;
@folded-codecolor: rgba(255, 255, 255, 1);
@bracketmatcher-backgroundcolor: rgba(255, 255, 255, 1);
@comment-color: rgba(156, 157, 162, 1);
@comment-fontstyle: normal;
@comment-value: 20;
@selector-color: rgba(18, 18, 18, 1);
@selector-fontstyle: normal;
@pseudo-color: rgba(236, 58, 42, 1);
@pseudo-fontstyle: normal;
@pseudo-weight: normal;
@selector-weight: normal;
@propertyname-color: rgba(214, 214, 214, 1);
@propertyname-fontstyle: normal;
@propertyname-weight: normal;
@propertyvalue-color: rgba(18, 18, 18, 1);
@propertyvalue-fontstyle: normal;
@propertyvalue-weight: normal;
@unit-color: rgba(214, 214, 214, 1);
@unit-fontstyle: normal;
@unit-weight: normal;
@number-color: rgba(18, 18, 18, 1);
@number-fontstyle: normal;
@number-weight: normal;
@punctuation-color: rgba(18, 18, 18, 1);
@punctuation-weight: bold;
@doctype-color: rgba(18, 18, 18, 1);
@doctype-fontstyle: normal;
@doctype-weight: bold;
@tag-color: rgba(250, 62, 45, 1);
@tag-fontstyle: normal;
@tag-weight: normal;
@attributename-color: rgba(250, 62, 45, 1);
@attributename-fontstyle: normal;
@attributename-weight: normal;
@quotationmark-color: rgba(18, 18, 18, 1);
@quotationmark-weight: bold;
@string-color: rgba(214, 214, 214, 1);
@string-fontstyle: italic;
@string-weight: normal;
@storage-color: rgba(66, 66, 66, 1);
@storage-fontstyle: normal;
@storage-weight: normal;
@storagemodifier-color: rgba(66, 66, 66, 1);
@storagemodifier-fontstyle: normal;
@storagemodifier-weight: normal;
@operator-color: rgba(214, 214, 214, 1);
@operator-weight: bold;
@functionname-color: rgba(240, 59, 43, 1);
@functionname-fontstyle: normal;
@functionname-weight: normal;
@consolelog-color: rgba(255, 38, 0, 1);
@consolelog-fontstyle: normal;
@consolelog-weight: normal;
@class-color: rgba(18, 18, 18, 1);
@class-fontstyle: normal;
@class-weight: normal;
@instance-color: rgba(18, 18, 18, 1);
@instance-fontstyle: normal;
@instance-weight: normal;
@regex-color: rgba(18, 18, 18, 1);
@regex-fontstyle: normal;
@regex-weight: normal;
@control-color: rgba(255, 255, 255, 1);
@control-fontstyle: normal;
@control-weight: normal;
@constant-color: rgba(18, 18, 18, 1);
@constant-weight: normal;
@boolean-color: rgba(238, 59, 42, 1);
@boolean-fontstyle: italic;
@boolean-weight: normal;
@module-color: rgba(18, 18, 18, 1);
@module-fontstyle: normal;
@module-weight: normal;
@namespace-color: rgba(255, 255, 255, 1);
@namespace-fontstyle: normal;
@namespace-weight: normal;
@type-color: rgba(255, 255, 255, 1);
@type-fontstyle: normal;
@type-weight: normal;
@identifier-color: rgba(18, 18, 18, 1);
@identifier-fontstyle: normal;
@identifier-weight: normal;
@gitgutter-lineaddedcolor: rgba(0, 249, 0, 1);
@gitgutter-linemodifiedcolor: rgba(255, 251, 0, 1);
@gitgutter-lineremovedcolor: rgba(255, 0, 0, 1);`
  }
}
